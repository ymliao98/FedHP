...
parser.add_argument('--decay_rate', type=float, default=1) #0.993
...
master_listen_port_base = 53500
p2p_listen_port_base = 56500
...
def main():
    ...
    for worker_idx, worker_config in enumerate(workers_config['worker_config_list']):
            for neighbor_idx, link in enumerate(adjacency_matrix[worker_idx]):
                if link == 1:
                    ...
                    worker_list[worker_idx].config.custom["neighbor_info"][neighbor_idx] = \
                            (neighbor_ip, p2p_port[worker_idx][neighbor_idx], p2p_port[neighbor_idx][worker_idx])
                    #worker_list[worker_idx].config.custom["average_weight"][neighbor_idx] = 1.0/worker_num
    ...
    for epoch_idx in range(1, 1+common_config.epoch):
        ...
        avg_train_loss = 0.0
        for worker in worker_list:
            worker_time, acc, test_loss,train_loss = worker.train_info
        ...
        local_steps_list,epoch_time,avg_waiting_time=update_E(worker_list,local_steps_list)
        #topology=update_T(worker_num)
        topology=adjacency_matrix
        update_W(topology,worker_list)

        total_resource+=Sum(computation_resource,local_steps_list)
        #total_bandwith=total_bandwith+Sum(bandwith_resource,compre_ratio_list)
        total_time+=epoch_time
        print(total_time,total_resource,avg_waiting_time)
        ...
        recoder.add_scalar('bandwith_time', avg_waiting_time, total_time)
        recoder.add_scalar('resource_epoch', total_resource, epoch_idx)
        recoder.add_scalar('bandwith_epoch', avg_waiting_time, epoch_idx)
        result_out.write('{} {:.2f} {:.2f} {:.2f} {:.4f} {:.4f} {:.4f}'.format(epoch_idx,total_time,avg_waiting_time,total_resource,avg_acc,avg_test_loss,avg_train_loss))
        result_out.write('\n')
...
def update_T1(worker_num):
    worker_selected=dict()
    for i in range(worker_num):
        worker_selected[i]=0
    topology = np.zeros((worker_num, worker_num), dtype=np.int)
    for i in range(worker_num):
        if worker_selected[i]<2:
            nei_list=random.sample(range(i+1,9),2-worker_selected[i])
            print(nei_list)
            for nei in nei_list:
                if worker_selected[nei]>=2:
                    nei=(nei+1)%10
                    continue
                topology[i][nei]=1
                topology[nei][i]=1
                worker_selected[nei]+=1
                worker_selected[i]+=1
...
def update_W(topology,worker_list):
    for worker in worker_list:
        ...
        worker.config.comm_neighbors_idx=list()
        for i in range(len(worker_list)):
            if topology[worker.idx][i]==1:
                worker.config.comm_neighbors_idx.append(i)
        worker.config.weight=get_weight(worker,worker_list)
        for i in range(len(worker_list)):
            if topology[worker.idx][i]==1:
                worker.config.comm_neighbors.append((i,worker.config.weight[i]))

def get_weight(worker,worker_list):
    weight=dict()
    sum=worker.config.local_steps
    for i in worker.config.comm_neighbors_idx:
        sum+=worker_list[i].config.local_steps
    for i in worker.config.comm_neighbors_idx:
        #print(neighbor_name)
        weight[i]=worker_list[i].config.local_steps/sum
    return weight

def update_E(worker_list,local_steps_list):
    ...
    for worker in worker_list:
        ...
        worker.config.local_steps=2
        #worker.config.local_steps=int(local_steps/2)+3
        local_steps_dict[worker.user_name]=worker.config.local_steps
        local_steps_list[worker.idx]=worker.config.local_steps
        sum_local_steps=sum_local_steps+worker.config.local_steps
    
    epoch_time=min_train_time*local_steps*0.8
    #epoch_time=max(train_time_list)*27
    for worker in worker_list:
        avg_wait_time+=abs(epoch_time-worker.config.train_time*worker.config.local_steps)

    avg_wait_time=avg_wait_time/float(len(worker_list))

    # for worker in worker_list:
    #     worker.config.weight=get_weight(worker,local_steps_dict)
        
    return local_steps_list,epoch_time,avg_wait_time
    ...
    try:
        ...
        for worker in worker_list:
            if action == "init":
                ...
                data=(worker.config.local_steps,worker.config.comm_neighbors)
                tasks.append(loop.run_in_executor(executor, worker.send_data,data))
